...
def ReadNumericalMatInfo(path, mat):
    ...
    infile =  open('%s/MaterialsDataBase/NumericalInformation/%s_Info.dat' %(path, mat), 'r')
    file = infile.readline()
    alat = file.split()[2]
    infile.close()
    alat = float(alat)/0.529177249
    #print (alat)
    
    with open('%s/MaterialsDataBase/NumericalInformation/%s_Info.dat' %(path, mat), 'r') as infile:
        copy = False
        for line in infile:
    ...
    CartesianCoordinate = CoordTransform.Cartesian(cell, atm)
    
    return CartesianCoordinate, atm, cell
